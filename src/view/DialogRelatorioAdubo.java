 ...
 * @author Walas Jhony
 */
public class DialogRelatorioAdubo extends JDialog {
    ...
    public DialogRelatorioAdubo() { }

    public DialogRelatorioAdubo(ArrayList<Adubo> lista) {
        ...
        textPreco = new JFormattedTextField("Preço");
        textPreco.setEditable(false);
        textPreco.setFont(Constantes.font);
        textPreco.setHorizontalAlignment(SwingConstants.CENTER);
        ...
        for (int i = 0; i < lista.size(); i++) {
            textNome = new JFormattedTextField(lista.get(i).getNome());
            textNome.setEditable(false);
            textNome.setFont(Constantes.font);
            textNome.setPreferredSize(dNome);

            textQuant = new JFormattedTextField(PontoPorVirgula(String.valueOf(lista.get(i).getQuant())));
            textQuant.setEditable(false);
            textQuant.setFont(Constantes.font);
            textQuant.setHorizontalAlignment(SwingConstants.CENTER);
            textQuant.setPreferredSize(dQuant);

            textPreco = new JFormattedTextField(PontoPorVirgula(String.valueOf(lista.get(i).getPreco())));
            textPreco.setEditable(false);
            textPreco.setFont(Constantes.font);
            textPreco.setHorizontalAlignment(SwingConstants.CENTER);
            textPreco.setPreferredSize(dPreco);
